  ...
  idx = {
    ...
    workspace = {
      ...
      onCreate = {
        build-flutter = ''
          ...
          # curl -sL https://github.com/OlyMahmudMugdho/shell-scripts/raw/main/setup-react-native-idx/script.sh | bash
        '';
        # react-native = '' 
        #     kill $(lsof -t -i:9101) && kill $(lsof -t -i:9102) && kill $(lsof -t -i:5555) && kill $(lsof -t -i:5037) && rm -rf * && socat -d -d TCP-LISTEN:5554,reuseaddr,fork TCP:$(cat /etc/resolv.conf | tail -n1 | cut -d " " -f 2):5554
        # '';
      };
    ...
    previews = {
      ...
      previews = {
        ...
        android = {
          command = ["flutter" "run" "--machine" "-d" "android" "-d" "localhost:6782"];
          manager = "flutter";
        };
      };
    };
